...
const _currentStartDateTime = ref<string>(await ScheduleStartTimeRetrieveService.execute())
...
export function useScheduleManager() {
    ...
    const getScheduleList : ComputedRef<CalculatedScheduleType[]> = computed(() => {
        const orderedSchedule = scheduleList.value.sort(
            (firstElement : ScheduleType, secondElement : ScheduleType) => firstElement.order - secondElement.order
        )
    ...
    const moveTask = (id: number, direction: MOVE_DIRECTION) => {
        ...
        if (
            ...
            || (scheduleIndex == lastScheduleIndex && direction == MOVE_DIRECTION.DOWN)
        ) return
